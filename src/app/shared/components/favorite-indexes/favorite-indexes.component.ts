...
@Component({
  ...
  styles: [`
    ...
    /* Layout a griglia: 4 colonne fisse su una riga */
    .box-row {
      display: grid;
...
export class FavoriteIndexesComponent implements OnChanges, OnDestroy {
  ...
  private startAuto() {
    if (this.autoTimer || this.totalPages <= 1) return;
    this.autoPlay = true;
  ...
  goToIndices() {
    void this.router.navigate(['/app/news-markets/indices']);
  }

  openIndex(idx: FavoriteIndex) {
    void this.router.navigate(['/app/news-markets/indices'], {queryParams: {q: idx.name}});
  }
